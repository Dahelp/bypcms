...
export default function Demo() {
  ...
  return <main className="demoApp">
    ...
    <section className="demoWorkspace"><header><div><span className="projectIcon">N</span><p><b>Northline Studio</b><small>Демонстрационный проект</small></p></div><span className="sessionBadge">● Данные удалятся после сессии</span><Link href="/">BYPCMS.RU ↗</Link></header>
      <div className="demoContent"><div className="demoHeading"><div><small>ПАНЕЛЬ УПРАВЛЕНИЯ / DEMO</small><h1>{titles[section]}</h1><p>Изучайте возможности BYPCMS без риска для рабочего проекта.</p></div>{(section==="overview"||section==="content")&&<button onClick={()=>setModal(true)}>＋ Создать страницу</button>}</div>
